...
const RePosting = () => {
  ...
  const projectSeq = location.state.projectSeq
  ...
  const postPutFetch = async () => {
    const res = await api.get(process.env.REACT_APP_API_URL + '/posting/' + projectSeq)
  ...
  const handleApplySubmit = async (event) => {
    ...
    if (
      ...
      await api
        .put(process.env.REACT_APP_API_URL + '/posting/' + projectSeq, req, config)
        .then((res) => {})
        .catch((err) => {
          console.log(err)
        ...
        <div>
          ...
          <div style={{ display: 'flex', marginBottom: '1em', marginLeft: '5em' }}>
            <div className="phone-section">
              ...
              <div style={{ width: '80%' }}>{1 && <TextField sx={inputStyle} value={subjectget} readonly />}</div>
            </div>
